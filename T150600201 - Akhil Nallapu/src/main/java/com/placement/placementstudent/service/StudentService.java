package com.placement.placementstudent.service;
...
import com.placement.placementstudent.repo.StudentRepo;
...
public class StudentService {
    ...
    public StudentEntity searchStudentById(long id) {
        Optional<StudentEntity> student = sr.findById((int) id);
        return student.orElseThrow(() -> new RuntimeException("Student not found with ID: " + id));
    }

    //Search Student by Hall Ticket Number
    public StudentEntity searchStudentByHallTicket(long ticketNo) {
        // Corrected method: Using the custom finder from StudentRepo
        Optional<StudentEntity> student = sr.findByHallTicketNo(ticketNo); 
    ...
    public boolean addCertificate(long studentId, String certificate) {
        StudentEntity student = sr.findById((int) studentId)
